            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-10 flex flex-col gap-y-5">
                ...
                <div class="item-card flex flex-row justify-between md:flex-row flex-col">
                    <div class="flex flex-row items-center gap-x-3">
                        ...
                        <p class="text-slate-500 text-sm">
                            {{$packageBooking->tour->category->name}}
                        </p>
                        </div>
                    </div> 
                    <br>
                    ...
                    <form action="{{route('admin.package_bookings.update', $packageBooking)}}" method="POST">
                        @csrf
                        @method('PATCH')
                        <button type="submit" class="font-bold py-4 px-6 bg-indigo-700 text-white rounded-full md:w-auto w-full text-center">
                            Approve Transaction
                        </button>
                    </form>
                    @endif
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div class="flex flex-col gap-y-4">
                      ...
                      <div class="flex flex-col">
                        <p class="text-slate-500 text-sm">Servis Fee</p>
                        <h3 class="text-indigo-950 text-xl font-bold">
                    ...
                    <div class="flex flex-col gap-y-4 md:w-1/2">
                      ...
                      <a href="{{Storage::url($packageBooking->proof)}}" class="glightbox w-full h-full aspect-[100/50] overflow-hidden" data-gallery="gallery1">
                        <img src="{{Storage::url($packageBooking->proof)}}" alt="" class="rounded-2xl object-cover w-full h-[200px] md:h-[300px]">
                      </a>
                    </div>
                  </div>
